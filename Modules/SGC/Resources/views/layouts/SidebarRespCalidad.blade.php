<!-- ======= MAIN SIDEBAR RESPONSABLE DE CALIDAD (AdminLTE & Bootstrap) ======= -->
        ...
        <div style="overflow-x: hidden; width: 100%;">
            ...
            <div class="px-3 pt-3 pb-2 sidebar-badge-container" style="overflow: hidden; width: 100%;">
                <div class="d-flex align-items-center gap-2 px-2.5 py-1.5 rounded-pill shadow-xs" style="background: rgba(57, 169, 0, 0.05); border: 1px solid rgba(57, 169, 0, 0.18); backdrop-filter: blur(8px); -webkit-backdrop-filter: blur(8px); min-width: 0; overflow: hidden;">
                    ...
                    <small class="fw-bold text-truncate" style="font-size: 9px; letter-spacing: 0.3px; color: #007832; font-family: 'Outfit', sans-serif; text-transform: uppercase; min-width: 0;">
                        Centro Agroindustrial • La Angostura
                    </small>
                </div>
            </div>
            ...
            <nav class="mt-2 px-3" style="overflow-x: hidden; width: 100%;">
                <ul class="nav nav-pills nav-sidebar flex-column gap-1" data-widget="treeview" role="menu" data-accordion="false" style="overflow-x: hidden; width: 100%;">
                    ...
                    <!-- 1. Inicio / Dashboard -->
                    @php
                        $isInicioActive = request()->routeIs('sgc.resp_calidad.dashboard') || (request()->routeIs('sgc.dashboard') && (auth()->check() && (auth()->user()->rol_id == 2 || str_contains(strtolower(auth()->user()->rol->nombre ?? ''), 'calidad'))));
                    @endphp
                    <li class="nav-item">
                        <a href="{{ route('sgc.resp_calidad.dashboard') }}" 
                    ...
                    @if(auth()->check() && (auth()->user()->hasSuperAdmin() || auth()->user()->tieneAccesoModulo('SGC', 'consultar_documento')))
                        @php
                            $isDocsActive = request()->routeIs('sgc.documentos.*');
                        @endphp
                        <li class="nav-item">
                    ...
                    @if(auth()->check() && (auth()->user()->hasSuperAdmin() || auth()->user()->tieneAccesoModulo('SGC', 'gestionar_versiones')))
                        @php
                            $isVersionesCalidadActive = request()->routeIs('sgc.versiones.*') || request()->routeIs('sgc.admin.versiones.*');
                        @endphp
                        <li class="nav-item">
                            <a href="{{ route('sgc.versiones.index') }}" 
                               class="nav-link d-flex align-items-center gap-3 {{ $isVersionesCalidadActive ? 'active' : '' }}" 
                               title="Versiones">
                    ...
                    @if(auth()->check() && (auth()->user()->hasSuperAdmin() || auth()->user()->tieneAccesoModulo('SGC', 'gestionar_catalogos')))
                        @php
                            $isCatalogosCalidadActive = request()->routeIs('sgc.catalogos.*') || request()->routeIs('sgc.procesos.*') || request()->routeIs('sgc.areas.*') || request()->routeIs('sgc.tipos-documento.*');
                        @endphp
                        <li class="nav-item">
                            <a href="{{ route('sgc.catalogos.index') }}" 
                               class="nav-link d-flex align-items-center gap-3 {{ $isCatalogosCalidadActive ? 'active' : '' }}" 
                               title="Catálogos SGC">
                    ...
                    @if(auth()->check() && (auth()->user()->hasSuperAdmin() || auth()->user()->tieneAccesoModulo('SGC', 'ver_reportes')))
                        @php
                            $isReportesActive = request()->routeIs('sgc.reportes.*');
                        @endphp
                        <li class="nav-item">
                            <a href="{{ route('sgc.reportes.index') }}" 
                               class="nav-link d-flex align-items-center gap-3 {{ $isReportesActive ? 'active' : '' }}" 
                               title="Reportes">
